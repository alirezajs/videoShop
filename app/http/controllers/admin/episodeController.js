...
class episodeController extends controller {
    ...
    async episodCourse(req, res, next) {
        try {
            let page = req.query.page || 1;
            let currentCourse = req.params.course;
            let episodes = await Episode.paginate({ course: currentCourse }, { page, sort: { createdAt: 1 }, limit: 20 });
            res.render('admin/episodes/index', { title: 'ویدیو ها', episodes });
        } catch (err) {
            next(err);
        }
